...
# Breadcrumb: [2025-09-04 08:44 EDT | 1744039440]
...
# Fixes architecture mismatch, sed error in graylog-server post-install, and redirection error (line 28).
...
# Log all output to file and console (simplified to avoid process substitution)
echo "----------------------------------------" | tee -a "$LOG_FILE"
echo "Starting Phase 3 setup at $(date '+%Y-%m-%d %H:%M:%S') (ARM64 detected)" | tee -a "$LOG_FILE"
echo "----------------------------------------" | tee -a "$LOG_FILE"

# Install required packages (OpenJDK 21 via APT for ARM64)
echo "Installing dependencies..." | tee -a "$LOG_FILE"
apt-get update | tee -a "$LOG_FILE"
apt-get install -y curl gnupg apt-transport-https lsb-release ca-certificates openjdk-21-jre-headless uuid-runtime nginx pwgen | tee -a "$LOG_FILE"

# Install OpenSearch via APT repository (fixes 403 direct download error)
echo "Installing OpenSearch $OPENSEARCH_VERSION via APT (ARM64)..." | tee -a "$LOG_FILE"
...
if apt-cache policy opensearch | grep -q "$OPENSEARCH_VERSION"; then
    apt-get install -y "opensearch=$OPENSEARCH_VERSION" | tee -a "$LOG_FILE"
else
    echo "Warning: $OPENSEARCH_VERSION not available; installing latest 2.x" | tee -a "$LOG_FILE"
    apt-get install -y opensearch | tee -a "$LOG_FILE"
fi

# Configure OpenSearch (disable security for simplicity; enable in production)
echo "Configuring OpenSearch..." | tee -a "$LOG_FILE"
...
systemctl enable opensearch | tee -a "$LOG_FILE"
systemctl start opensearch | tee -a "$LOG_FILE"

# Install MongoDB via APT repository (supports ARM64)
echo "Installing MongoDB 7.0 via APT..." | tee -a "$LOG_FILE"
...
apt-get update | tee -a "$LOG_FILE"
apt-get install -y mongodb-org | tee -a "$LOG_FILE"
systemctl enable mongod | tee -a "$LOG_FILE"
systemctl start mongod | tee -a "$LOG_FILE"

# Install Graylog repository
echo "Installing Graylog $GRAYLOG_VERSION repository..." | tee -a "$LOG_FILE"
wget -4 --timeout=30 --tries=3 -O graylog-repo.deb "$REPO_URL" | tee -a "$LOG_FILE"
dpkg -i graylog-repo.deb || { echo "Graylog repository installation failed" | tee -a "$LOG_FILE"; exit 1; }
rm graylog-repo.deb
apt-get update | tee -a "$LOG_FILE"

# Preconfigure Graylog server.conf to avoid sed error
echo "Preconfiguring Graylog to avoid sed error..." | tee -a "$LOG_FILE"
...
echo "Installing Graylog $GRAYLOG_VERSION..." | tee -a "$LOG_FILE"
if ! apt-get install -y graylog-server | tee -a "$LOG_FILE"; then
    echo "Warning: Graylog installation encountered errors (likely sed issue). Attempting to fix..." | tee -a "$LOG_FILE"
    dpkg --configure -a | tee -a "$LOG_FILE"
    apt-get install -f -y | tee -a "$LOG_FILE"
fi

# Start and enable Graylog
systemctl enable graylog-server | tee -a "$LOG_FILE"
systemctl start graylog-server | tee -a "$LOG_FILE"

# Install and configure Nginx
echo "Configuring Nginx as reverse proxy..." | tee -a "$LOG_FILE"
cat <<EOF >"$NGINX_CONF"
server {
    listen 80;
...
nginx -t || { echo "Nginx configuration test failed" | tee -a "$LOG_FILE"; exit 1; }
...
for svc in opensearch mongod graylog-server nginx; do
    if ! systemctl is-active --quiet "$svc"; then
        echo "Error: $svc service not running" | tee -a "$LOG_FILE"
        exit 1
    else
        echo "$svc service is running" | tee -a "$LOG_FILE"
    fi
done

# Test Graylog API
echo "Testing Graylog API..." | tee -a "$LOG_FILE"
if curl -s -f http://127.0.0.1:9000/api/system | grep -q "cluster_id"; then
    echo "Graylog API is accessible" | tee -a "$LOG_FILE"
else
    echo "Error: Graylog API not accessible" | tee -a "$LOG_FILE"
    exit 1
fi

echo "----------------------------------------" | tee -a "$LOG_FILE"
echo "Phase 3 setup completed successfully at $(date '+%Y-%m-%d %H:%M:%S')" | tee -a "$LOG_FILE"
echo "Access Graylog at http://$(hostname -I | awk '{print $1}'):80 with username 'admin' and password 'admin123'" | tee -a "$LOG_FILE"
echo "Log saved to $LOG_FILE" | tee -a "$LOG_FILE"
echo "Password secret: $PASSWORD_SECRET (save securely)" | tee -a "$LOG_FILE"
echo "----------------------------------------" | tee -a "$LOG_FILE"# Breadcrumb: [2025-09-04 08:42 EDT | 1744039320]
# Description: Installs and configures Nginx as a reverse proxy and Graylog as a syslog server
# for monitoring NETGEAR M4300 switches on ARM64 Debian Trixie. Uses APT repo for OpenSearch to fix 403 download error.
# Supports ARM64; installs OpenSearch 2.11.1 via repo (or latest 2.x if unavailable).

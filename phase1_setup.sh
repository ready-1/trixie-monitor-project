...
if command -v nvim >/dev/null 2>&1 && command -v fail2ban-client >/dev/null 2>&1; then
    ...
    apt install -y --no-install-recommends ifupdown rsyslog git neovim htop rsync curl wget net-tools sudo tmux sysstat iotop tcpdump nmap logwatch fail2ban yamllint mutt
